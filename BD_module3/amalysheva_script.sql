...
select job_industry_category, count(*) from module2.customer group by job_industry_category order by count(*) desc;
...
with v as (select distinct t.customer_id, 
...
max(t.transaction_date) over(partition by customer_id order by transaction_date)- min(t.transaction_date) over(partition by customer_id order by transaction_date) as diff
from module2.transaction t
order by diff desc)
